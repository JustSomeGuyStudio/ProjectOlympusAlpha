...
struct FDragonData_AimSolver_SkeletonInput
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = SkeletalControl, meta = (DisplayName = "Bone Input", PinVisibleByDefault))
...
public:
	...
	UPROPERTY(EditAnywhere, Category = CoreInputData, BlueprintReadWrite, meta = (DisplayName = "Use FName bone input",EditCondition = "!Use_Advanced_Input_Mode", PinHiddenByDefault))
	...
	* The head bone.
	*/
	UPROPERTY(EditAnywhere, Category = CoreInputData, meta = (DisplayName = "Start Bone (Eg:- Head)", EditCondition = "!Use_FName_Bone_Input && !Use_Advanced_Input_Mode", PinHiddenByDefault))
		FBoneReference EndSplineBone;

	/*
	* The last bone to be influenced. (Eg:- Pelvis or chest)
	*/
	UPROPERTY(EditAnywhere, Category = CoreInputData, meta = (DisplayName = "End Bone", EditCondition = "!Use_FName_Bone_Input && !Use_Advanced_Input_Mode", PinHiddenByDefault))
...
* The head bone.
*/
	UPROPERTY(EditAnywhere, Category = CoreInputData, meta = (DisplayName = "(FNAME) Start Bone (Eg:- Head)", EditCondition = "Use_FName_Bone_Input && !Use_Advanced_Input_Mode", PinHiddenByDefault))
		FName Name_EndSpline_Bone;

	/*
	* The last bone to be influenced. (Eg:- Pelvis or chest)
	*/
	UPROPERTY(EditAnywhere, Category = CoreInputData, meta = (DisplayName = "(FNAME) End Bone", EditCondition = "Use_FName_Bone_Input && !Use_Advanced_Input_Mode", PinHiddenByDefault))
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = CoreInputDataType2, meta = (DisplayName = "Set entire aim bone chain manually ?", PinHiddenByDefault))
	bool Use_Advanced_Input_Mode = false;


	/*
	 *
	 * NEW FEATURE! This is a very flexible and advanced input mode.
	 ...
	 * The bone order here don't need to follow any strict hierarchy based on your rig. Write them based on the visual order of the bone.
 	 */
	UPROPERTY(EditAnywhere, Category = CoreInputDataType2, meta = (DisplayName = "Aiming Bone Chain (Index 0 = EndBone/Root, LastIndex = StartBone/Head)",EditCondition = "Use_Advanced_Input_Mode", PinHiddenByDefault))
...
*/
	...
	/*
	 * Just like the Limbs Max Clamp, but you can control the clamp for each arm of your hand input. The index of the limb max clamp is tied to the index of the hand array input.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = AdvancedClampingSettings, meta = (DisplayName = "Per Limb Max Clamp Array (Optional)", PinHiddenByDefault))
...
*/
	...
	/*
	* If using mocap or any use case that involves altering the forward axis during gameplay, this can be enabled to dynamically alter the arm and leg poles along with the change in axis.
	...
	*/
	...
	bool Use_Reference_Forward_Axis = true;
