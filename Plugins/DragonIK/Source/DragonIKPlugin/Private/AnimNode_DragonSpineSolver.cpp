...
void FAnimNode_DragonSpineSolver::GetAnimatedPoseInfo(FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if ((owning_skel->GetWorld()->WorldType != EWorldType::EditorPreview || Work_Outside_PIE) && test_counter > 4)
	{
		...
		FTransform Pelvis_Updated_Transform;
				...
				else
				{
					...
					else if (complexity_type == ESolverComplexityPluginEnum::VE_Simple && !is_snake)
					{
						...
						if (i == 0)
						{
							...
							Pelvis_Updated_Transform = updated_transform;
						...
						if(use_type2_input)
						{
							if(i > 1 && i < NumTransforms - 1)
							{
								...
								FTransform Original_Pelvis_Transform = MeshBases.GetComponentSpaceTransform(combined_indices[0]);
								FTransform Original_Chest_Transform = MeshBases.GetComponentSpaceTransform(combined_indices[combined_indices.Num()-1]);

								FTransform Modded_Pelvis_Transform = Pelvis_Updated_Transform;
								FTransform Modded_Chest_Transform = AnimatedBoneTransforms[combined_indices.Num()-1].Transform;

								FVector body_to_chest_look = (Modded_Pelvis_Transform.GetLocation() - Modded_Chest_Transform.GetLocation()).GetSafeNormal();
								FVector body_to_chest_orig_look = ((Original_Pelvis_Transform.GetLocation()) - (Original_Chest_Transform.GetLocation()) ).GetSafeNormal();

								FQuat diff_rot = FQuat::FindBetweenNormals(body_to_chest_orig_look,body_to_chest_look);
								FTransform Diff_Rot_Transform = Pelvis_Updated_Transform;
								
								Diff_Rot_Transform.SetRotation(MeshBases.GetComponentSpaceTransform(combined_indices[0]).GetRotation()*diff_rot);
								pelvis_difference = MeshBases.GetComponentSpaceTransform(combined_indices[0]).Inverse() * (Diff_Rot_Transform);

								updated_transform.SetLocation((MeshBases.GetComponentSpaceTransform(combined_indices[i])*pelvis_difference).GetLocation());
								updated_transform.SetRotation(Original_AnimatedBoneTransforms[i].Transform.GetRotation()*pelvis_difference.GetRotation());
								OutBoneTransforms.Add(FBoneTransform(combined_indices[i], UKismetMathLibrary::TLerp(MeshBases.GetComponentSpaceTransform(combined_indices[i]), updated_transform, Total_spine_alphas[i])));
						...
						if (i > 0 && i == NumTransforms - 1 && !Ignore_Chest_Solve)
						{
							...
							//updated_transform.SetLocation(updated_transform.GetLocation()+FVector(0,0,1000));
							OutBoneTransforms.Add(FBoneTransform(combined_indices[i], UKismetMathLibrary::TLerp(MeshBases.GetComponentSpaceTransform(combined_indices[i]), updated_transform, Total_spine_alphas[i])));
						}
					...
					if(use_type2_input)
					{
						...
						{
							for (int extra_spine_index = 0; extra_spine_index < spine_Feet_pair[spine_index].Spine_Children_Involved.Num(); extra_spine_index++)
							{

								

								if(combined_indices[i] == spine_Feet_pair[spine_index].Spine_Involved.CachedCompactPoseIndex)
								{

									FTransform bone_updated_difference = MeshBases.GetComponentSpaceTransform(combined_indices[i]).Inverse() * updated_transform;

									FTransform Bone_Override = MeshBases.GetComponentSpaceTransform(spine_Feet_pair[spine_index].Spine_Children_Involved[extra_spine_index].CachedCompactPoseIndex) * bone_updated_difference;

									OutBoneTransforms.Add(FBoneTransform(spine_Feet_pair[spine_index].Spine_Children_Involved[extra_spine_index].CachedCompactPoseIndex, UKismetMathLibrary::TLerp(MeshBases.GetComponentSpaceTransform(spine_Feet_pair[spine_index].Spine_Children_Involved[extra_spine_index].CachedCompactPoseIndex), Bone_Override, Total_spine_alphas[i])));

									//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " CHILD SPINE FOLLOW! "+FString::SanitizeFloat(spine_Feet_pair[spine_index].Spine_Children_Involved[extra_spine_index].CachedCompactPoseIndex.GetInt()));
								}
...
void FAnimNode_DragonSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if(Enable_Solver == false || Alpha == 0)
	{
		test_counter = 0;
	}
	...
	if (Character_Actor)
	{
		Dragon_Trace_Manager = owning_skel->GetOwner()->FindComponentByClass<UDragonIKTraceManagerComponent>();
		...
		solver_time_reference += owning_skel->GetWorld()->DeltaTimeSeconds;
	}
	else
	{
	...
	if (test_counter < 60 || solver_time_reference < 0.5f)
	{

		Formatted_Shift_Speed = 100;

		Maximum_Formated_Dip_Height = 100;

		Maximum_Formated_Dip_Height_Chest = 100;


		Formatted_Location_Lerp = 100;

		Formatted_Rotation_Lerp = 100;

		Formatted_Trace_Lerp = 100;

		Formatted_Snake_Lerp = 100;
...
void FAnimNode_DragonSpineSolver::CoreEvaluation(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (solve_should_fail == false && (FVector(0, 0, 0) - Output.AnimInstanceProxy->GetActorTransform().GetScale3D()).Size() > 0 && spine_Feet_pair.Num() > 0 && FAnimWeight::IsRelevant(ActualAlpha) && IsValidToEvaluate(Output.AnimInstanceProxy->GetSkeleton(), Output.AnimInstanceProxy->GetRequiredBones()) && Output.ContainsNaN() == false)
	{
		...
		//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "spine_Feet_pair "+FString::SanitizeFloat(spine_Feet_pair.Num()) + " ---- "+FString::SanitizeFloat(Total_spine_heights.Num()));
			...
			for (int i = 0; i < spine_Feet_pair.Num(); i++)
			{
				...
				float chest_distance = FMath::Abs(((bonetraceTransform.GetLocation() - RoottraceTransform.GetLocation()) * character_direction_vector_CS).Size());
	...
	if (spine_hit_pairs.Num() > 0)
	{
		...
		for (int k = 0; k < spine_hit_pairs.Num(); k++)
		{
			for (int i = 0; i < spine_hit_pairs[k].RV_Feet_Hits.Num(); i++)
			{
				...
				if (UpVectorNormalized_HeightCheck(spine_hit_pairs[k].RV_Feet_Hits[i].ImpactPoint , (owning_skel->GetComponentLocation())) > Maximum_Feet_Distance * component_scale * virtual_scale)
				{
					if (spine_hit_pairs[k].RV_Feet_Hits[i].bBlockingHit)
						atleast_one_hit = false;
				}
			...
			if (use_crosshair_trace_also_for_fail_distance)
			{
				...
				for (int i = 0; i < spine_hit_pairs[k].RV_Feet_Hits.Num(); i++)
				{
					...
					if (UpVectorNormalized_HeightCheck(spine_hit_pairs[k].Parent_Back_Hit.ImpactPoint , (owning_skel->GetComponentLocation())) > Maximum_Feet_Distance * component_scale * virtual_scale)
					{
					...
					if (UpVectorNormalized_HeightCheck(spine_hit_pairs[k].Parent_Front_Hit.ImpactPoint , (owning_skel->GetComponentLocation())) > Maximum_Feet_Distance * component_scale * virtual_scale)
					{
					...
					if (UpVectorNormalized_HeightCheck(spine_hit_pairs[k].Parent_Left_Hit.ImpactPoint , (owning_skel->GetComponentLocation())) > Maximum_Feet_Distance * component_scale * virtual_scale)
					{
					...
					if (UpVectorNormalized_HeightCheck(spine_hit_pairs[k].Parent_Right_Hit.ImpactPoint ,(owning_skel->GetComponentLocation())) > Maximum_Feet_Distance * component_scale * virtual_scale)
					{
...
float FAnimNode_DragonSpineSolver::UpVectorNormalized_HeightCheck(FVector vectorA,FVector vectorB)
{


	FVector Impact_Original_Abs_Dir = (vectorB - vectorA)*(UKismetMathLibrary::TransformDirection(owner_skel_w_transform , character_direction_vector_CS));

	return Impact_Original_Abs_Dir.Size();
}
...
bool FAnimNode_DragonSpineSolver::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	...
	if(use_type2_input)
	{
		...
		for (int i = 0; i < dragon_input_type2_data.FeetRelatedData.Num(); i++)
		{

			FBoneReference common_spine_bone = FBoneReference(dragon_input_type2_data.FeetRelatedData[i].CommonSpineBone);
			common_spine_bone.Initialize(RequiredBones);

			if(common_spine_bone.IsValidToEvaluate(RequiredBones) == false)
			{
				feet_is_true = false;
			}
			
			
			for (int j = 0; j < dragon_input_type2_data.FeetRelatedData[i].Feet_Pair_Array.Num(); j++)
			{
				...
				FBoneReference foot_bone_ref = FBoneReference(dragon_input_type2_data.FeetRelatedData[i].Feet_Pair_Array[j].Feet_Bone_Name);
	...
	if (is_snake || (dragon_bone_data.FeetBones.Num() == 0 && !use_type2_input))
	{
		feet_is_true = true;
	...
	if(use_type2_input)
	{
		...
		if(dragon_input_type2_data.Spine_Structure.Num() < 2)
			solve_should_fail = true;

		for (int i = 0; i < dragon_input_type2_data.Spine_Structure.Num(); i++)
		{
			if(dragon_input_type2_data.Spine_Structure[i].IsNone())
				solve_should_fail = true;



			FBoneReference spine_bone = FBoneReference(dragon_input_type2_data.Spine_Structure[i]);
			spine_bone.Initialize(RequiredBones);

			if(spine_bone.IsValidToEvaluate(RequiredBones) == false)
			{
				solve_should_fail = true;
			}
	...
	else
	{
		...
		if(dragon_input_data.Start_Spine.IsNone())
			solve_should_fail = true;


		if(!RequiredBones.BoneIsChildOf(FBoneReference(Chest_ref).BoneIndex, FBoneReference(Pelvis_ref).BoneIndex))
			solve_should_fail = true;
	}
	//feet_is_empty = false;
...
void FAnimNode_DragonSpineSolver::InitializeBoneReferences(FBoneContainer& RequiredBones)
{
	...
	bool allow_initialization_flow = (!use_type2_input&&!dragon_input_data.Start_Spine.IsNone()) || (use_type2_input&&dragon_input_type2_data.Spine_Structure.Num() > 1);

	if(allow_initialization_flow)
	{
		...
		if(use_type2_input)
		{
			...
			dragon_bone_data.Start_Spine = FBoneReference(dragon_input_type2_data.Spine_Structure[dragon_input_type2_data.Spine_Structure.Num()-1]);
			dragon_bone_data.Start_Spine.Initialize(RequiredBones);

			dragon_bone_data.Pelvis = FBoneReference(dragon_input_type2_data.Spine_Structure[0]);
		...
		if (solve_should_fail == false)
		{
			...
			if(use_type2_input)
			{
				Total_spine_bones = dragon_input_type2_data.Spine_Structure;

				
				
				

				for(int bone_count=0;bone_count < dragon_input_type2_data.FeetRelatedData.Num();bone_count++)
				{
					FDragonData_SpineFeetPair data = FDragonData_SpineFeetPair();
					data.Spine_Involved = FBoneReference(dragon_input_type2_data.FeetRelatedData[bone_count].CommonSpineBone);
					...
					TArray<FBoneReference> spine_children_bones = TArray<FBoneReference>();
					
					for(int spine_index=0;spine_index < dragon_input_type2_data.FeetRelatedData[bone_count].ChildFollowBones.Num();spine_index++)
					{

						FBoneReference spine_child = FBoneReference(dragon_input_type2_data.FeetRelatedData[bone_count].ChildFollowBones[spine_index]);
						spine_child.Initialize(*SavedBoneContainer);


						if(spine_child.IsValidToEvaluate())
						{
							spine_children_bones.Add(spine_child);
						}
					
					}

					data.Spine_Children_Involved = spine_children_bones;
					
					for(int feet_bone_index=0;feet_bone_index < dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array.Num();feet_bone_index++)
					{

						FDragonData_FootData foot_temp_data = dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array[feet_bone_index];
						FBoneReference foot_bone = FBoneReference(foot_temp_data.Feet_Bone_Name);
						foot_bone.Initialize(*SavedBoneContainer);
						data.Associated_Feet.Add(foot_bone);
						
						data.Feet_Heights.Add(foot_temp_data.Feet_Heights);
						data.feet_trace_offset.Add(foot_temp_data.Feet_Trace_Offset);
						...
						if (!foot_temp_data.Thigh_Bone_Name.IsEqual("None"))
						{

							FBoneReference thigh_bone_ref = FBoneReference(foot_temp_data.Thigh_Bone_Name);
							thigh_bone_ref.Initialize(*SavedBoneContainer);
							data.Associated_Thighs.Add(thigh_bone_ref);
						}
			...
			if(use_type2_input)
			{
				...
				for(int bone_count=0;bone_count < dragon_input_type2_data.FeetRelatedData.Num();bone_count++)
				{
					for (int32 i = 0; i < dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array.Num(); i++)
					{

						for (int32 j = 0; j < dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array.Num(); j++)
						{
							if (i != j)
							{
								if (dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array[i].Feet_Bone_Name == dragon_input_type2_data.FeetRelatedData[bone_count].Feet_Pair_Array[j].Feet_Bone_Name)
								{
									solve_should_fail = true;
								}
							}
			...
			//if (test_counter < 10)
			if(Total_spine_alphas.Num() == 0)
			{
				if(use_type2_input)
					Total_spine_alphas.AddDefaulted(dragon_input_type2_data.Spine_Structure.Num());
				else
			...
			for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
			{
			//	GEngine->AddOnScreenDebugMessage(-1, 5.01f, FColor::Red, "Total_spine_bones "+spine_Feet_pair[i].Spine_Involved.BoneName.ToString());
				...
				//if (test_counter < 10)
				if(spine_hit_pairs[i].RV_Feet_Hits.Num() == 0)	
				{
			...
			{
				if (dragon_bone_data.Start_Spine == dragon_bone_data.Pelvis)
					solve_should_fail = true;
			...
			for (int32 i = 0; i < Total_spine_bones.Num(); i++)
			{
				...
				//	GEngine->AddOnScreenDebugMessage(-1, 4.01f, FColor::Red, "--bone_ref " + bone_ref.BoneName.ToString() );
			...
			//if (test_counter < 10)
			if(spine_between_transforms.Num() == 0)	
			{
			...
			if (spine_Feet_pair.Num() > 1 )
			{
				...
				if(use_type2_input)
				{
					...
					for(int bone_i=0;bone_i<Total_spine_bones.Num();bone_i++)
					{
						
						FCompactPoseBoneIndex BoneIndex = FCompactPoseBoneIndex(owning_skel->GetBoneIndex(Total_spine_bones[bone_i]));
			...
			{
				if (dragon_bone_data.Pelvis == dragon_bone_data.Start_Spine)
				{
					solve_should_fail = true;
				}
	...
	if (owning_skel->GetOwner() && (trace_timer_count > current_trace_interval && trace_distance_legal))
	{
			...
			//	if (RV_Anti_Checker.GetComponent())
			{
				...
				if (response_channel == ECollisionResponse::ECR_Block)
				{
					...
					if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type)
					{
						...
						if(use_trace_manager_component)
						{
							FName Trace_Identifier = FName("S"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
							UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), New_StartPoint, endpoint,0,true, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color,Trace_Identifier,bone_text);
						}
						else
						{
						...
						{//(startpoint-endpoint).Size()
							...
							{
								FName Trace_Identifier = FName("S"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
								UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), New_StartPoint, endpoint,Trace_Radius,false, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color,Trace_Identifier,bone_text);
							}
							else
							{
		...
		else
		{
			...
			if (ducking_mode)
			{
				...
				{
					FName Trace_Identifier = FName("S"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
					UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint, endpoint,0,true, Ducking_Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color,Trace_Identifier,bone_text);
				}
				else
				{
			...
			else
			{
				...
				if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type)
				{
					...
					{
						FName Trace_Identifier = FName("S"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
						UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint, endpoint,0,true, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color,Trace_Identifier,bone_text);
					}
					else
					{
						UKismetSystemLibrary::LineTraceSingle(owning_skel->GetOwner(), startpoint, endpoint, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color);
					...
					{//(startpoint-endpoint).Size()
						...
						{
							FName Trace_Identifier = FName("S"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
							UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint, endpoint,Trace_Radius,false, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, debug_color,Trace_Identifier,bone_text);
						}
						else
						{
...
void FAnimNode_DragonSpineSolver::FABRIK_BodySystem(FComponentSpacePoseContext& Output, FBoneReference TipBone, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int i = 0; i < combined_indices.Num(); i++)
	{
		...
		if (i > 0 && i < combined_indices.Num() - 1)
		{
			...
			if((i-1) < spine_hit_between.Num() && (i-1) < spine_between_points.Num())
			{
			...
			if( (i-1) < spine_between_offseted_transforms.Num())
...
void FAnimNode_DragonSpineSolver::Snake_ImpactRotation(int origin_point_index, FTransform& OutputTransform, FCSPose<FCompactPose>& MeshBases)
{
	...
	if (origin_point_index == 0)
	{
		...
		if (atleast_one_hit)
		{
			...
			spine_RotDifference[origin_point_index] = position_Based_rot;
			
			spine_RotDifference[origin_point_index] = FRotator(FMath::Clamp<float>(position_Based_rot.Pitch,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)),FMath::Clamp<float>(position_Based_rot.Yaw,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)),FMath::Clamp<float>(position_Based_rot.Roll,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)));
...
float FAnimNode_DragonSpineSolver::Normalize_Towards_Up_Axis(FVector Input_Vector)
{
	FVector Front_Back_Diff =  (Input_Vector)*character_direction_vector_CS;
	float dot_dir = FVector::DotProduct(Front_Back_Diff.GetUnsafeNormal(),character_direction_vector_CS);
	dot_dir = (dot_dir>0 ? 1 :-1);

	return (Front_Back_Diff.Size() * dot_dir);
}
		...
		if (atleast_one_hit && spine_hit_pairs[origin_point_index].Parent_Spine_Hit.bBlockingHit)
		{
			...
			*/
			...
			//if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 2)
			{
...

				{
					...
					for (int32 i = 0; i < spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num(); i++)
					{
						...
						float inv_transform_z = Normalize_Towards_Up_Axis(inv_transform.GetLocation());

						float Z_Offset_z = Normalize_Towards_Up_Axis(Z_Offset);

						float Z_Reverse_Offset_z = Normalize_Towards_Up_Axis(Z_Reverse_Offset);
						
						//if (inv_transform.GetLocation().Z < Z_Offset.Z)
						if (inv_transform_z < Z_Offset_z)
						{
							Z_Offset = inv_transform.GetLocation();
						}
						...
						//if (reversed_gravity)
						{
							...
							if (inv_transform_z > Z_Reverse_Offset_z)	
							{
			...
			if (spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() == 2)
			{
				...
				float FeetHit_0_Z = Normalize_Towards_Up_Axis( owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[opposite_index].RV_FeetHit_Points[0]));
				float FeetHit_1_Z = Normalize_Towards_Up_Axis( owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[opposite_index].RV_FeetHit_Points[1]));

				if (FeetHit_0_Z > FeetHit_1_Z)
					feet_difference_offset_opp = FeetHit_1_Z;
				else
					feet_difference_offset_opp = FeetHit_0_Z;


				//Normalize_Towards_Up_Axis
				//	if (atleast_one_hit)
...
/*
			FVector SimpleDirections[] = {FVector(0, 0, 1), FVector(0, 1, 0), FVector(1, 0, 0),FVector(0, 0, -1), FVector(0, -1, 0), FVector(-1, 0, 0)};
			...
			{
				float DotProduct = FVector::DotProduct( UKismetMathLibrary::TransformDirection(owner_skel_w_transform,character_direction_vector_CS), SimpleDirections[s_i].GetUnsafeNormal());

				//	GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " DotProduct : " + FString::SanitizeFloat(DotProduct));
			...
			SimpleDirections[ClosestIndex] = FVector(FMath::Abs<float>(SimpleDirections[ClosestIndex].X),FMath::Abs<float>(SimpleDirections[ClosestIndex].Y),FMath::Abs<float>(SimpleDirections[ClosestIndex].Z));
*/
		
			float AC_Cross_Value = 0;

			//float AC_Cross_Value = ((owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point) - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point))*character_direction_vector_CS).Size();

			if (flip_forward_and_right)
			{

				AC_Cross_Value = Normalize_Towards_Up_Axis (owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Point) - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index]. Parent_Right_Point));
			}
			else
			{
				
				AC_Cross_Value = Normalize_Towards_Up_Axis((owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point) - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index]. Parent_Front_Point)));
				
			}

			float OG_Cross_Value = Normalize_Towards_Up_Axis((Feet_Opposite_Mid_Point - Feet_Mid_Point));
				...
				*/
				...
				else
				{
					...
					//	new_location += character_direction_vector_CS * (Feet_Mid_Point.Z - Feet_Opposite_Mid_Point.Z) * Slanted_Height_Offset * FMath::Clamp<float>((spine_median_result / 10) * slop_tolerance, 0, 1);
						...
						new_location += character_direction_vector_CS * Normalize_Towards_Up_Axis(Feet_Mid_Point - Feet_Opposite_Mid_Point) * Slanted_Height_Offset * FMath::Clamp<float>((spine_median_result / 10) * slop_tolerance, 0, 1);
						...
						new_location = UKismetMathLibrary::VLerp(lerp_data - relative_root_offset,new_location,Chest_Influence_Alpha);
...
*/
			...
			FVector Absolute_Up_Vector = FVector(FMath::Abs<float>(character_direction_vector_CS.X),FMath::Abs<float>(character_direction_vector_CS.Y),FMath::Abs<float>(character_direction_vector_CS.Z));

			FVector Impact_Original_Abs_Dir = (final_transformation.GetLocation() - lerp_data)*( Absolute_Up_Vector);
			final_transformation.SetLocation(lerp_data+Impact_Original_Abs_Dir);
			...
			if (origin_point_index == 0)
			{
				...
				if (owning_skel->GetOwner())
				{
					...
					if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || (Use_Fake_Pelvis_Rotations || is_this_biped) || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
					{
						...
						if (spine_hit_pairs[origin_point_index].Parent_Front_Hit.bBlockingHit && spine_hit_pairs[origin_point_index].Parent_Back_Hit.bBlockingHit)
						{
							...
							*/ {								
								...
								//ORIGINAL: Intensifier_forward = (owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Pelvis_ForwardRotation_Intensity_INPUT;								
								
								//float Front_Back_Z_Distance = ((owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point) - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point))*character_direction_vector_CS).Size();                                								                                							
                                //Intensifier_forward = Front_Back_Z_Distance * Pelvis_ForwardRotation_Intensity_INPUT;                                					
                                
                                
                                FVector Front_Back_Diff =  ((owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point) - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point)))*character_direction_vector_CS;
                                float dot_dir = FVector::DotProduct(Front_Back_Diff.GetUnsafeNormal(),character_direction_vector_CS);
                                dot_dir = (dot_dir>0 ? 1 :-1);
                                float Front_Back_Z_Distance = Front_Back_Diff.Size() * dot_dir;
                                Intensifier_forward = FMath::Clamp<float>(Front_Back_Z_Distance*Pelvis_ForwardRotation_Intensity_INPUT, -10000, 10000);
					...
					else
					{
						...
						FVector Front_Back_Diff = (Feet_Mid_Point - Feet_Opposite_Mid_Point)*character_direction_vector_CS;
						float dot_dir = FVector::DotProduct((Feet_Mid_Point - Feet_Opposite_Mid_Point).GetUnsafeNormal(),character_direction_vector_CS);
						dot_dir = (dot_dir>0 ? 1 :-1);
						float Front_Back_Z_Distance = Front_Back_Diff.Size() * dot_dir;
						Intensifier_forward = FMath::Clamp<float>(Front_Back_Z_Distance*Pelvis_ForwardRotation_Intensity_INPUT, -10000, 10000);
						
						//ORIGINAL : Intensifier_forward = FMath::Clamp<float>((Feet_Mid_Point.Z - (Feet_Opposite_Mid_Point.Z)) * Pelvis_ForwardRotation_Intensity_INPUT, -10000, 10000) * -1;
...
*/
				...
				//FQuat Orig_DiffNormal = FQuat::FindBetweenNormals(FVector(0,0,1),FVector(0,1,0));
				//FQuat DiffNormal = FQuat::FindBetweenNormals(( Forward_Direction_Vector ),((Feet_Opposite_Mid_Point-Feet_Mid_Point)*Intensifier_forward).GetUnsafeNormal());

				FVector Forward_Altered_Vector = ((Forward_Direction_Vector*100 + character_direction_vector_CS*-Intensifier_forward) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat DiffNormal = FQuat::FindBetweenNormals(( Forward_Direction_Vector ),(Forward_Altered_Vector));

				
				position_Based_rot = FRotator(DiffNormal);

				position_Based_rot.Normalize();
				...
				if (owning_skel->GetOwner())
				{
					...
					if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || Use_Fake_Chest_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
					{
						...
						//	forward_intensity = (owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Point).Z - owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Point).Z) * Chest_ForwardRotation_Intensity_INPUT;
							...
							dot_dir = (dot_dir>0 ? 1 :-1);
							float Front_Back_Z_Distance = Front_Back_Diff.Size() * dot_dir;
							forward_intensity = FMath::Clamp<float>(Front_Back_Z_Distance*Chest_ForwardRotation_Intensity_INPUT, -10000, 10000);
						}
						else
							forward_intensity = 0;
					...
					else
					{
						...
						FVector Front_Back_Diff = ((Feet_Opposite_Mid_Point - Feet_Mid_Point))*character_direction_vector_CS;
						float dot_dir = FVector::DotProduct((Feet_Opposite_Mid_Point - Feet_Mid_Point).GetUnsafeNormal(),character_direction_vector_CS);
						dot_dir = (dot_dir>0 ? 1 :-1);
						float Front_Back_Z_Distance = Front_Back_Diff.Size() * dot_dir;
						forward_intensity = FMath::Clamp<float>(Front_Back_Z_Distance*Chest_ForwardRotation_Intensity_INPUT, -10000, 10000);
...
*/
				...
				///
				FVector Forward_Altered_Vector = ((Forward_Direction_Vector*100 + character_direction_vector_CS*-forward_intensity) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat DiffNormal = FQuat::FindBetweenNormals(( Forward_Direction_Vector ),(Forward_Altered_Vector));
				position_Based_rot = FRotator(DiffNormal);
				////
				///
				...
				//	spine_RotDifference[origin_point_index] = FRotator(FMath::Clamp<float>(position_Based_rot.Pitch,-60,60),FMath::Clamp<float>(position_Based_rot.Yaw,-60,60),FMath::Clamp<float>(position_Based_rot.Roll,-60,60));
					spine_RotDifference[origin_point_index] = FRotator(FMath::Clamp<float>(position_Based_rot.Pitch,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)),FMath::Clamp<float>(position_Based_rot.Yaw,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)),FMath::Clamp<float>(position_Based_rot.Roll,-FMath::Abs<float>(RotationLimit),FMath::Abs<float>(RotationLimit)));
				...
				if (atleast_one_hit)
				{
						...
						//						OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, OutputTransform.GetLocation().Z), final_transformation.GetLocation() + FVector(0, 0, base_offset), 1 - FMath::Exp(-2 * owning_skel->GetWorld()->GetDeltaSeconds()), FMath::Clamp<float>(1/FMath::Abs(OutputTransform.GetLocation().Z - final_transformation.GetLocation().Z) , 0.1,1)* Formatted_Location_Lerp * 20));
					{
						...
						if (!(Ignore_Lerping || test_counter < 10))
						{
							...
							FVector Final_Final_Transform = final_transformation.GetLocation() + offset_worldspace;
							
						//	FVector Transform_Original_Offset = ( Final_Final_Transform - OutputTransform.GetLocation() )*(UKismetMathLibrary::TransformDirection(owner_skel_w_transform,Absolute_Up_Vector));
						//	OutputTransform.SetLocation(Final_Final_Transform+Transform_Original_Offset);
						...
						//	FVector Transform_Original_Offset = ( OutputTransform.GetLocation() - Final_Final_Transform )*(UKismetMathLibrary::TransformDirection(owner_skel_w_transform,Absolute_Up_Vector));
							FVector Transform_Original_Offset = ( OutputTransform.GetLocation() - Final_Final_Transform )*(Absolute_Up_Vector);

							OutputTransform.SetLocation(Final_Final_Transform + Transform_Original_Offset);
							

							OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(OutputTransform.GetLocation(), Final_Final_Transform, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Formatted_Location_Lerp * 0.05f));
							


							//OutputTransform.SetLocation(UKismetMathLibrary::VInterpTo(OutputTransform.GetLocation(), final_transformation.GetLocation() + offset_worldspace, 1 - FMath::Exp(-Smooth_Factor * owning_skel->GetWorld()->GetDeltaSeconds()), Formatted_Location_Lerp * 0.05f));
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor_Direct(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int32 TransformIndex = 1; TransformIndex < NumTransforms; TransformIndex++)
	{
		...
		*/
		...
		{
			//Then add chain link with the respective bone
			Chain.Add(DragonChainLink(BoneCSPosition - character_direction_vector_CS*Fabrik_Height_Offet, BoneLength, BoneIndex, TransformIndex));
