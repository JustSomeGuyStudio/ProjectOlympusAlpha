...
protected:
  ...
  UFUNCTION(BlueprintNativeEvent, Category = "General Movement Component")
  bool SV_CheckSyncDataForcedNetUpdate_Custom(const FGMC_Move& CurrentMove, const FGMC_Move& PreviousMove) const;
  virtual bool SV_CheckSyncDataForcedNetUpdate_Custom_Implementation(const FGMC_Move& CurrentMove, const FGMC_Move& PreviousMove) const { return false; }
...
public:
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Networking|Server", meta = (ClampMin = "1", UIMin = "5", UIMax = "30"))
  ...
  int32 PredictedClientNetUpdateFrequency{15};
  ...
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Networking|Server", meta = (ClampMin = "1", UIMin = "1", UIMax = "20"))
  /// Determines how many remote moves will be processed in one server frame at most. In bad network conditions lower values introduce less additional latency
  /// (but have a larger impact on server performance) while higher values are likely to produce better simulation results.
  int32 RemoteMoveProcessingChunkSize{10};

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Networking|Server", meta = (ClampMin = "0", UIMin = "10", UIMax = "50"))
  ...
  /// of elements in the buffer is below the threshold again.
  int32 MaxNumBufferedRemoteMoves{30};
  ...
  /// synchronised between server and client for any pawn that is rolled back. Additionally, pawns can only participate in the rollback if they use a delay
  /// based interpolation mode.
  bool bRollBackServerPawns{true};

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Networking|Lag Compensation")
  /// When enabled, all simulated proxies of type AGMC_Pawn are set back in time for replay on the client. This can significantly improve replay accuracy (thus
  /// making corrections less noticeable) at the cost of an increased client performance overhead. Pawns can only participate in the rollback if they use a
  /// delay based interpolation mode.
