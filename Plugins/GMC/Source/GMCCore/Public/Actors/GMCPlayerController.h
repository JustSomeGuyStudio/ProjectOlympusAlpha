...
public:
  ...
  /// How many reliable RPCs should be reserved for the actor at the least outside when attempting to send adaptive delay parameters to the client.
  ...
  /// The camera manager update may be deferred during a smoothed correction on the client.
  ///
  ...
  /// Called after the correction buffer is swapped at the end of a frame for the autonomous proxy during a smooth correction and for non-predicted clients.
  /// Used to perform additional setup on the camera before rendering.
  ///
  ...
  void CL_ManualUpdateCamera(float DeltaTime);
  virtual void CL_ManualUpdateCamera_Implementation(float DeltaTime);

  /// Enqueues the passed delay packet for transmission to the client on the next tick.
  ///
  /// @param        DelayPacket    The packet to enqueue.
  /// @returns      void
  void SV_EnqueueAdaptiveDelay(const struct FGMC_AdaptiveDelayServerPacket& DelayPacket);
  ...
  void CL_SendAdaptiveDelayParams(const TArray<FGMC_AdaptiveDelayServerPacket>& AdaptiveDelayParams) const;
  void CL_SendAdaptiveDelayParams_Implementation(const TArray<FGMC_AdaptiveDelayServerPacket>& AdaptiveDelayParams) const;
  ...
  UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Client Time Sync", meta =
...
private:
  ...
  double CL_SyncedWorldTime{0.};

  /// What the time was during the last frame so we can identify inconsistent timestamps (can happen after the time was synced).
  double CL_SyncedTimeLastFrame{0.};

  /// If set only a fraction of the delta time will be added to the client world time during the next client time update. This is done to preemptively bring the
  /// client time closer to the server time.
  bool CL_bSlowWorldTime{false};
  ...
  bool CL_bSpeedUpWorldTime{false};
  ...
  bool CL_bDoNotUpdateWorldTime{false};

  /// Saves past discrepancies of the client time from the server time to generate an average deviation.
  TGMC_CircularNumericBuffer<double, TIME_DISCREPANCY_BUFFER_SIZE> CL_TimeDiscrepancyBuffer{};

  /// The adaptive delay parameters that are sent to the client at regular intervals.
  TArray<FGMC_AdaptiveDelayServerPacket> SV_AdaptiveDelayPackets{};
};
