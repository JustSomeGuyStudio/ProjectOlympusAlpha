...
void AGMC_Aggregator::Tick(float DeltaTime)
{
...

  {
    ...
    for (int32 Index = 0; Index < Controllers.Num(); ++Index)
    {
      ...
      {
        Controller->TickActor(DeltaTime, ELevelTick::LEVELTICK_All, Controller->PrimaryActorTick);
      }
    }
...

  {
    ...
    for (int32 Index = 0; Index < Pawns.Num(); ++Index)
    {
      ...
      {
        Pawn->TickActor(DeltaTime, ELevelTick::LEVELTICK_All, Pawn->PrimaryActorTick);
      }
    }
...

  {
    ...
    for (int32 Index = 0; Index < RollbackActors.Num(); ++Index)
    {
      ...
      {
        RollbackActor->TickActor(DeltaTime, ELevelTick::LEVELTICK_All, RollbackActor->PrimaryActorTick);
      }
    }
...

  {
    ...
    for (int32 Index = 0; Index < MeshComponents.Num(); ++Index)
    {
      ...
      {
        MeshComponent->TickComponent(DeltaTime, ELevelTick::LEVELTICK_All, &MeshComponent->PrimaryComponentTick);
      }
    }
...
{
  // Tick order:
  int32 GMCReplicationComponent_SimulatedProxy = 0;
  int32 GMCMovementUtilityComponent_SimulatedProxy = 1;
  int32 GMCOrganicMovementComponent_SimulatedProxy = 2;
  int32 GMCGoldSrcMovementComponent_SimulatedProxy = 3;
  ...
  {
    return MovementComponentCast->IsSimulatedProxy() ? GMCGoldSrcMovementComponent_SimulatedProxy : GMCGoldSrcMovementComponent;
  }

  if (const auto& MovementComponentCast = Cast<UGMC_OrganicMovementCmp>(MovementComponent))
  {
    return MovementComponentCast->IsSimulatedProxy() ? GMCOrganicMovementComponent_SimulatedProxy : GMCOrganicMovementComponent;
  }

  if (const auto& MovementComponentCast = Cast<UGMC_MovementUtilityCmp>(MovementComponent))
  {
    return MovementComponentCast->IsSimulatedProxy() ? GMCMovementUtilityComponent_SimulatedProxy : GMCMovementUtilityComponent;
  }

  if (const auto& MovementComponentCast = Cast<UGMC_ReplicationCmp>(MovementComponent))
  {
    return MovementComponentCast->IsSimulatedProxy() ? GMCReplicationComponent_SimulatedProxy : GMCReplicationComponent;
  }
