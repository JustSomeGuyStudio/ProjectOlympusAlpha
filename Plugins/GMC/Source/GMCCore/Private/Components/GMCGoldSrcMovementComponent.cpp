...
void UGMC_GoldSrcMovementCmp::AdjustVelocityFromHitAirborne_Implementation(const FHitResult& Hit, float DeltaSeconds)
{
...
FVector UGMC_GoldSrcMovementCmp::ComputeSlideVector(
  const FVector& Delta,
  float Time,
...
float UGMC_GoldSrcMovementCmp::SlideAlongSurface(
  const FVector& Delta,
  float Time,
  const FVector& Normal,
  FHitResult& Hit,
...
void UGMC_GoldSrcMovementCmp::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
...
{
  if (MinRequiredZ < 0.f || !IsMovingOnGround())
  {
    return;
  }
