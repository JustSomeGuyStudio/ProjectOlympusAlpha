...
void UGMC_OrganicMovementCmp::ClampToValidValues()
{
  ...
  FloorTraceLength = FMath::Clamp(FloorTraceLength, MaxStepDownHeight + MAX_DISTANCE_TO_FLOOR + UE_KINDA_SMALL_NUMBER, UE_BIG_NUMBER);
...
{
  // The flag should have been processed at this point, so this is a good place to reset it for the next movement update.
  bReceivedUpwardForce = false;
...
bool UGMC_OrganicMovementCmp::UpdateMovementModeDynamic_Implementation(FGMC_FloorParams& Floor, float DeltaSeconds)
{
  ...
  if (IsMovingOnGround())
  {
    ...
    const bool bShapeHitWithinStepDownHeight =
      bHasValidShapeData && Floor.GetShapeDistanceToFloor() <= MaxStepDownHeight + MAX_DISTANCE_TO_FLOOR;
    const bool bLineHitWithinStepDownHeight =
      bHasValidLineData && Floor.GetLineDistanceToFloor() <= MaxStepDownHeight + MAX_DISTANCE_TO_FLOOR;
...
void UGMC_OrganicMovementCmp::RaiseFromGround(FGMC_FloorParams& Floor)
{
  ...
  {
    DeltaZ -= Floor.GetShapeDistanceToFloor() - 0.01;
...
bool UGMC_OrganicMovementCmp::MaintainDistanceToFloor(FGMC_FloorParams& Floor)
{
...
void UGMC_OrganicMovementCmp::BotDirectMove_Implementation(const FVector& PathVelocity, const FGMC_RootMotionVelocitySettings& RootMotionMetaData, float DeltaSeconds)
{
  ...
  else if (IsAirborne() && IsAffectedByGravity() && PathDirection.Z == 0.)
  {
    ...
    const FVector NewVelocityZ = {0., 0., FMath::Clamp(Velocity.Z, -UE_DOUBLE_BIG_NUMBER, 0.) + GetGravityZ() * DeltaSeconds};
...
void UGMC_OrganicMovementCmp::MoveWithBaseVelocity_Implementation(float DeltaSeconds)
{
  ...
  const FRotator RotationDeltaRotator = ToAngularVelocityRotator(AngularBaseVelocity * DeltaSeconds);
...
FVector UGMC_OrganicMovementCmp::MoveAlongFloor(const FVector& LocationDelta, FGMC_FloorParams& Floor, float DeltaSeconds)
{
  ...
  const bool bStartedWithinLineMaxStepDownHeight = Floor.HasValidLineData() && Floor.GetLineDistanceToFloor() <= MaxStepDownHeight + MAX_DISTANCE_TO_FLOOR;
  ...
  else if (MoveHitResult.IsValidBlockingHit())
  {
    ...
    else
    {
      ...
      else if (bDidStepUp && CurrentLocation.Z <= HitStartLocation.Z + UE_DOUBLE_KINDA_SMALL_NUMBER)
      {
...
bool UGMC_OrganicMovementCmp::StepUp(const FVector& LocationDelta, const FHitResult& BarrierHit, const FGMC_FloorParams& Floor)
{
  ...
  if (MaxStepUpHeight <= 0.f || StepLocationDelta.IsNearlyZero(UE_DOUBLE_KINDA_SMALL_NUMBER))
  {
    return false;
  }
...
UPrimitiveComponent* UGMC_OrganicMovementCmp::FindActorBase_Implementation()
{
  ...
  if (Hit.ImpactPoint.Z >= CurrentLocation.Z)
  {
    FLog(VeryVerbose, "Found actor base \"Null\" (impact point not on the lower part of the root collision).")
    return nullptr;
  }
  ...
  if (FMath::Abs(CurrentLocation.Z - Hit.Location.Z) <= BasedMovement.GetMaxHeight())
  {
    const auto& BaseComponent = GetBaseComponent(BasedMovement.GetUseAttachParentAsBase());
    FLog(VeryVerbose, "Found actor base \"%s\".", *GetObjectNameWithOuter(BaseComponent))
    return BaseComponent;
...
FVector UGMC_OrganicMovementCmp::HandleSlopeBoosting(
...
void UGMC_OrganicMovementCmp::OnMovementModeChanged_Implementation(EGMC_MovementMode PreviousMovementMode)
{
  ...
  else if (IsAirborne() && PreviousMovementMode == EGMC_MovementMode::Grounded)
  {
...
void UGMC_OrganicMovementCmp::CL_CheckMontageStatusAfterReplay(const FGMC_MontageTracker& PreReplayMontageTracker)
{
  ...
  if (MontageTracker.Montage != PreReplayMontageTracker.Montage)
  {
    if (MontageTracker.Montage)
    {
      ...
      const float MontageLength = AnimInstance->Montage_PlayWithBlendIn(
        MontageTracker.Montage,
    ...
    if (PreReplayMontageTracker.Montage)
    {
      ...
      AnimInstance->Montage_Stop(MontageReplication.MontagePrediction.CorrectionBlendTime, nullptr/*stop all active montages*/);
      return;
    }
  ...
  if (MontageTracker.MontagePosition != PreReplayMontageTracker.MontagePosition)
  {
    ...
    AnimInstance->Montage_SetPosition(MontageTracker.Montage, MontageTracker.MontagePosition);
  }

  if (MontageTracker.MontagePlayRate != PreReplayMontageTracker.MontagePlayRate)
  {
    // The play rate of the montage has changed.
    AnimInstance->Montage_SetPlayRate(MontageTracker.Montage, MontageTracker.MontagePlayRate);
  }
  ...
  if (MontageTracker.bMontagePaused != PreReplayMontageTracker.bMontagePaused)
  {
    if (MontageTracker.bMontagePaused)
    {
      ...
      AnimInstance->Montage_Pause(MontageTracker.Montage);
      return;
    }
    ...
    if (PreReplayMontageTracker.bMontagePaused)
    {
      ...
      gmc_ck(!MontageTracker.bMontagePaused)
      AnimInstance->Montage_Resume(MontageTracker.Montage);
      return;
    }
...
)
{
  ...
  if (AnimInstance->Montage_Play(MontageToPlay, PlayRate, EMontagePlayReturnType::MontageLength, StartPosition, bStopAllMontages) > 0.f)
  {
...
)
{
    ...
    if (PreviousMontageSimulated)
    {
      ...
      if (EndMontageTimerSimulated > CompletionTolerance)
      {
        if (AnimInstance->GetActiveInstanceForMontage(PreviousMontageSimulated))
        {
          ...
          AnimInstance->Montage_Stop(BlendOutTime, PreviousMontageSimulated);
        }
  ...
  if (ActiveMontageInstance && ActiveMontageInstance->IsValid())
  {
    ...
    else
    {
      if (!bIsPlaying)
      {
        AnimInstance->Montage_Resume(ActiveMontage);
      }
    }
