...
void UGMC_ReplicationCmp::SetUpdatedComponent(USceneComponent* NewUpdatedComponent)
{
  ...
  {
    if (IsValid(GMCAggregator) && GMCAggregator->bEnableAggregateTick)
    {
...
void UGMC_ReplicationCmp::PreTick()
{
  ...
  ComponentStatus.ElapsedRealTimeSinceSpawn += DeltaTime;
...
void UGMC_ReplicationCmp::UpdateAutonomousProxyPawn()
{
  ...
  if ((bUseClientPrediction ? MoveHistory.Num() > 1 : CL_MoveExecutionAux.NonPredictedMoves.Num() > 1) && !bMoveDiscarded)
  {
...
void UGMC_ReplicationCmp::ManagePrerequisiteTicks()
{
  ...
  // Gather all actors of type AGMC_Pawn.
...
void UGMC_ReplicationCmp::SmoothMovement(float DeltaTime, double& OutSimTime, int32& OutTargetIdx, TArray<int32>& OutSkippedStateIndices)
{
  ...
  const bool bShouldExtrapolate = !bDidInterpolate && ExtrapolationMode != EGMC_ExtrapolationMode::None && MaxExtrapolationDistance > 0.;
...
bool FGMC_Move::SV_ShouldReplicate()
{
  ...
  {
    if (NetInfo.OwningComponent->SV_PredictedClientNetSerializationAux.ShouldUpdate())
    {
...
bool UGMC_ReplicationCmp::FPredictedClientNetSerializationAux::ShouldUpdate() const
{
