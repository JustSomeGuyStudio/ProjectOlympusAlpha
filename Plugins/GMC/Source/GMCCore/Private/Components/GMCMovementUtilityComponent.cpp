...
float UGMC_MovementUtilityCmp::PreProcessMoveDeltaTime_Implementation(float RealDeltaTime) const
{
  ...
  return FMath::Clamp(RealDeltaTime * WorldSettings->GetEffectiveTimeDilation(), MIN_DELTA_TIME, UE_BIG_NUMBER);
}
...
) const
{
  ...
  )
  {
    ...
    ReturnValue = AnimInstance->Montage_Play(MontageToPlay, PlayRate, ReturnValueType, StartPosition, true);
...
) const
{
  ...
  )
  {
    ...
    const auto& AnimInstance = Mesh->GetAnimInstance();
    gmc_ck(IsValid(AnimInstance))
    ReturnValue = AnimInstance->Montage_PlayWithBlendIn(MontageToPlay, BlendIn, PlayRate, ReturnValueType, StartPosition, true);
    CFLog(ReturnValue == 0.f, Error, "Montage \"%s\" failed to play.", *MontageToPlay->GetName())
    gmc_ck(ReturnValue > 0.f)
    return ReturnValue;
  }
...
) const
{
  ...
  )
  {
    ...
    const auto& AnimInstance = Mesh->GetAnimInstance();
    gmc_ck(IsValid(AnimInstance))
    ReturnValue = AnimInstance->Montage_PlayWithBlendSettings(MontageToPlay, BlendInSettings, PlayRate, ReturnValueType, StartPosition, true);
    CFLog(ReturnValue == 0.f, Error, "Montage \"%s\" failed to play.", *MontageToPlay->GetName())
    gmc_ck(ReturnValue > 0.f)
    return ReturnValue;
  }
...
) const
{
  ...
  AnimInstance->Montage_JumpToSection(SectionName, MontageTracker.Montage);
}
...
) const
{
  ...
  AnimInstance->Montage_JumpToSectionsEnd(SectionName, MontageTracker.Montage);
}
...
) const
{
  ...
  AnimInstance->Montage_SetPlayRate(MontageTracker.Montage, NewPlayRate);
}
...
) const
{
  ...
  AnimInstance->Montage_SetPosition(MontageTracker.Montage, NewPosition);
}
